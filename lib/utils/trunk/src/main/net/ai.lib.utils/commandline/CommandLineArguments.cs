...
//    
//    Arguments may have long and short names. 
//
//    Long names must be prefiexed with "--":
//
//        program --long-arg
//    
//    Short names must be prefixed with "-" and can be combined together:
//    
//    program -a -b -c:- 
//    program -abc:- 
// 
...
//    using Utilities;
//
//    [CommandLine(HelpText="Some help text for the whole class.")]
//    class WCArguments
//    {
...
namespace ai.lib.utils.commandline
{
    using System;
    ...
    public class ArgumentAttribute : Attribute
    {
        ...
        /// Set to null means use the default long name (name of the field).
    ...
    /// '-' prefix only the argument value is specified.
    ...
    /// <summary>
    /// Parameters for the whole command line.
    /// </summary>
    ...
    /// <summary>
    /// Command line definition that supports standard arguments like -d.
    /// 
    /// The order of processing is the following:
    /// <para>1. Profile</para>
    /// <para>2. Definition of properties.</para>
    /// </summary>
    public class StandardCmdLine
    {
        /// <summary>
        /// Loads a profile. Properties are automatically added to Props.Global. If no value is specified, tries to 
        /// load a default profile, but throws no exception if the file not found.
        /// </summary>
        /// <remarks>A default name ends with a space on purpose, it is an indication that 
        /// the user specified no profile. </remarks>
        [ArgumentAttribute(ArgumentType.Multiple, ShortName = "", LongName = "profile", DefaultValue = "profile.xml ",
            HelpText = "Loads a profile.")]
        public string ProfileFile = null;

        public Profile Profile = null;

        /// <summary>
        /// Defines a property and adds it to Props.Global.
        /// </summary>
        [ArgumentAttribute(ArgumentType.Multiple, ShortName = "d", LongName = "define-prop",
            HelpText = "Defines a property and adds it to Props.Global")]
        public string[] DefineVars = null;


        internal bool ApplyStandardArguments(ErrorReporter errorReporter)
        {
            bool success = true;

            success &= LoadProfile(errorReporter);

            Regex reVar = new Regex("^\\s*([^\\s^=]+\\s*)=(.*)$");
            foreach (string varSpec in DefineVars)
            {
                Match m = reVar.Match(varSpec);
                if (!m.Success)
                {
                    errorReporter(String.Format("Wrong variable definition fomat: {0}, expected 'var=value'", varSpec));
                    success = false;
                }
                else
                {
                    Props.Global.Set(m.Groups[1].Value, m.Groups[2].Value);
                }
            }
            return success;
        }

        private bool LoadProfile(ErrorReporter errorReporter)
        {
            bool isDefaultProfileUsed = ProfileFile.EndsWith(" ");
            ProfileFile = ProfileFile.Trim();
            bool fileExists = File.Exists(ProfileFile);
            if (!fileExists)
            {
                if (isDefaultProfileUsed)
                    return true;
                errorReporter(String.Format("Profile {0} does not exist", ProfileFile));
                return false;
            }
            Profile = XmlSerializerExt.Deserialize<Profile>(ProfileFile);
            Props.Global.UpdateFrom(Profile.Properties);
            return true;
        }
    }
    ...
    /// indicating that arguments without a '-' prefix will be parsed as that argument.
    ///
    ...
    {
        /// <summary>
        /// Reads the argument array from Environment and cuts off the 1st argument - 
        /// the name of the application.
        /// </summary>
        /// <returns>Array of arguments without the name of the application.</returns>
        public static string[] GetArgumentsFromEnvironment()
        {
            string[] args = Environment.GetCommandLineArgs();
            // Cut off the 1st argument - the name of our executable.
            string[] argsNoApp = new string[args.Length - 1];
            Array.Copy(args, 1, argsNoApp, 0, args.Length - 1);
            return argsNoApp;
        }
        /// <summary>
        ...
        /// <returns> true if no errors were detected and no help requested. </returns>
        ...
        public Parser(Type argumentSpecification, ErrorReporter reporter)
        {
            ...
            object[] attributes = argumentSpecification.GetCustomAttributes(typeof(CommandLineAttribute), false);
            if (attributes.Length == 1)
            {
                classAttributes = (CommandLineAttribute) attributes[0];
            }
            ...
            foreach (FieldInfo field in argumentSpecification.GetFields())
            {
                if (!field.IsStatic && !field.IsInitOnly && !field.IsLiteral)
                {
                    ...
                    if(attribute == null)
                    {
                        // Ignore simple fields with no command-line specific attributes.
                        continue;
                    }
                    ...
                    if (argument.Length > 0)
                    {
                        switch (argument[0])
                        {
                            case '-':
                                ...
                                int endIndex = argument.IndexOfAny(new char[] { ':', '+' }, startIndex);
                                endIndex = endIndex == -1 ? argument.Length : endIndex;
                                // '-' can be a part of the name, therefore we need a special handling for it.
                                if (Regex.Match(argument, ".?-$").Success)
                                {
                                    endIndex = argument.Length - 1;
                                }
                                string option = argument.Substring(startIndex, endIndex - startIndex);
                                string optionArgument;
                                if (option.Length + startIndex == argument.Length)
                                {
                                    optionArgument = null;
                                }
                                else if (argument.Length > startIndex + option.Length && argument[startIndex + option.Length] == ':')
                                {
                                    optionArgument = argument.Substring(option.Length + startIndex + 1);
                                }
                                else
                                {
                                    optionArgument = argument.Substring(option.Length + startIndex);
                                }

                                if (!isLongName && option.Length > 1)
                                {
                                    // We have combined short-names.
                                    for (int i = 0; i < option.Length - 1; ++i)
                                    {
                                        hadError |= !FindAndEvaluateArgument(
                                            argument, option.Substring(i, 1), null, destination);
                                    }
                                    // For the last option allow to process optional argument.
                                    option = option.Substring(option.Length - 1);
                                }

                                hadError |= !FindAndEvaluateArgument(argument, option, optionArgument, destination);
                                break;
                            case '@':
        ...
        private bool FindAndEvaluateArgument(string argument, string option, string optionArgument, object destination)
        {
            bool success = true;
            ...
            {
                success = arg.SetValue(optionArgument, destination);
            }
            return success;
        }
        ...
        public bool Parse(string[] args, object destination)
        {
            ...
            StandardCmdLine scmd = destination as StandardCmdLine;
            if (scmd != null && !hadError)
            {
                hadError = !scmd.ApplyStandardArguments(this.reporter);
            }
        ...
        public string GetUsageString(int screenWidth)
        {
            ...
            if (classAttributes != null && classAttributes.HasHelpText)
            {
                string classHelp = classAttributes.HelpText;
                for (int pos = 0; pos < classHelp.Length; )
                {
                    string line = classHelp.Substring(pos, Math.Min(classHelp.Length - pos, screenWidth));
                    builder.Append(line);
        ...
        private class Argument
        {
            public Argument(ArgumentAttribute attribute, FieldInfo field, ErrorReporter reporter)
            {
                ...
                if (field.FieldType == typeof(PropString))
                {
                    Debug.Assert(!this.HasDefaultValue || (this.defaultValue.GetType() == typeof(string)),
            ...
            public bool Finish(object destination)
            {
                ...
                {
                    if (this.Type == typeof(PropString))
                    {
                        this.field.SetValue(destination, new PropString((string)this.DefaultValue));
                    }
                    else
                    {
            ...
            private bool ParseValue(Type type, string stringData, out object value)
            {
                try
                {
                    ...
                    else if (type == typeof(PropString))
                    {
                        string rawValue = stringData ?? "";
                        value = new PropString(rawValue);
                        return true;
                    }
                    else if (type == typeof(bool))
                    {
                        if (String.IsNullOrEmpty(stringData) || stringData == "+")
                        {
                            value = true;
                            return true;
                        }
                    ...
                    {
                        value = Int64.Parse(stringData);
                        return true;
                    }
                    else if (type == typeof(UInt64))
                    {
                        value = UInt64.Parse(stringData);
                        return true;
                    }
                    else
                    {
            ...
            public string SyntaxHelp
            {
                get
                {
                    ...
                    else
                    {
                        ...
                        else if (valueType == typeof(PropString))
                        {
        ...
        private ErrorReporter reporter;
        CommandLineAttribute classAttributes;
    }
}
//
//    Author of the original version: peterhal@microsoft.com
