...
{
    /// <summary>
    /// Unit tests for VerifyCondStrategy. 
    /// </summary>
    [TestFixture]
    public unsafe class VerifyCondStrategy_Test
    {
        #region Tests
        ...
        public void Test_Kuhn()
        {
            ...
            for (_heroPos = 0; _heroPos < gd.MinPlayers; ++_heroPos)
            {
                ...
                if (_heroPos == 0)
                {
                    ...
                    Assert.AreEqual(expTextBegin, errorText.Substring(0, expTextBegin.Length), string.Format("Hero pos {0}", _heroPos));
                    st.Nodes[18].Probab -= 0.1;
        ...
        void OnNodeBegin(StrategyTree tree, Context[] stack, int depth)
        {
            ...
            if (tree.Nodes[n].IsPlayerAction(_heroPos))
            {
                ...
                {
                    tree.Nodes[n].Probab = 1.0 / _childrenCount[stack[depth - 1].NodeIdx];
                }
            }
        }
        ...
        string _outDir = UTHelper.MakeAndGetTestOutputDir(Assembly.GetExecutingAssembly(), "algorithms/VerifyCondtStrategyTree_Test");
